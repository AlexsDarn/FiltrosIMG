...
from tkinter import NW, Image, canvas, PhotoImage, messagebox
...
def otsu():
    ...
    for i in range(len(histogram)):
        ...
        if clase1 != 0 and clase2 != 0:
            m1 = mean[i] / clase1
            m2 = (mt - mean[i]) / clase2
            sigma_b2 = (clase1 * (m1 - mt) * (m1 - mt) + clase2 * (m2 - mt) * (m2 - mt))
    ...
    im_otsu.save(ruta_base + 'otsu.gif') 
    imagen_l = PhotoImage(file = ruta_base + 'otsu.gif')
    global Otsu    
    Otsu = canvas.create_image(600, 160, anchor=NW, image=imagen_l)
    ventana.mainloop()